...
def build_split(dataset_root: Path, split: str) -> tuple[int, int]:
    ...
    output_dir = dataset_root / split / "ocr_sorted"
...
def sort_reading_order(records):
    ...
    median_height = sorted(x["h"] for x in items)[len(items)//2]

    row_threshold = max(8, median_height * 0.6)
    ...
    for item in items:
        ...
        row_center = sum(x["cy"] for x in rows[-1]) / len(rows[-1])
    ...
    for row in rows:
        ...
        ordered.extend(x["record"] for x in row)
